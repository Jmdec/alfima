...
function StatTicker({ total }: { total: number }) {
  return (
    <div className="overflow-hidden whitespace-nowrap border-y border-red-900/40 py-2.5"
...
// ── Hero Right Panel — pure typography + decoration, zero images ──────────────
function HeroPanel({ total, loading }: { total: number; loading: boolean }) {
  ...
  return (
    <div className="relative flex flex-col gap-4 select-none">
      ...
      <p className="text-red-400/50 text-[10px] font-bold tracking-[0.35em] uppercase relative z-10"
      ...
      <div className="grid grid-cols-2 gap-3 relative z-10">
        {stats.map((s, i) => (
          ...
          >
            <span className="text-3xl font-black text-white leading-none" style={{ fontFamily: 'Georgia, serif' }}>
              {s.value}
            </span>
            <span className="text-white/35 text-xs leading-tight" style={{ fontFamily: 'monospace' }}>
              {s.label}
            </span>
          </div>
        ))}
...
function PropertiesPageInner() {
  ...
  const fetchProperties = async (filters?: any, page = 1, quick = quickFilter, sort = sortBy) => {
    setLoading(true);
    try {
  ...
  const handlePageChange  = (p: number) => { setCurrentPage(p); fetchProperties(activeFilters, p, quickFilter, sortBy); window.scrollTo({ top: 0, behavior: 'smooth' }); };
      ...
      >
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          ...
          <div className="flex items-center gap-3 mb-5">
            ...
            <span className="text-red-300/60 text-[10px] font-bold tracking-[0.35em] uppercase"
              style={{ fontFamily: 'monospace' }}>
              Alfima Realty Inc. · Property Listings
            </span>
          </div>

          {/* Two-column layout: headline+search LEFT, mosaic RIGHT */}
          <div className="grid grid-cols-1 lg:grid-cols-[1fr_420px] gap-8 items-start">
            ...
            {/* RIGHT: Stats panel or filtered preview */}
            <div className="hidden lg:block pb-0">
              {isFiltered ? (
                /* Filtered results preview panel */
                <div className="rounded-2xl overflow-hidden flex flex-col" style={{ minHeight: '340px', background: 'rgba(255,255,255,0.08)', border: '1px solid rgba(255,255,255,0.12)' }}>
                  {/* Header */}
                  <div className="flex items-center justify-between px-4 py-3.5" style={{ background: 'rgba(255,255,255,0.07)', borderBottom: '1px solid rgba(255,255,255,0.08)' }}>
                    <div className="flex items-center gap-2.5">
                      <div className="w-1.5 h-8 rounded-full" style={{ background: 'linear-gradient(to bottom, #e74c3c, #96281b)' }} />
                      <div>
                        <p className="text-white/40 text-[9px] font-bold tracking-[0.3em] uppercase" style={{ fontFamily: 'monospace' }}>Top results</p>
                  ...
                  {/* Property preview rows */}
                  <div className="flex flex-col flex-1">
                    {loading ? (
                      [...Array(4)].map((_, i) => (
                        <div key={i} className="flex items-center gap-3 px-4 py-3 animate-pulse" style={{ borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
                      ...
                      const rawPrice = Number(isRent
                        ? (p.price_per_month ?? p.pricePerMonth ?? p.price ?? 0)
                        : (p.price ?? 0));
                      const imageUrl = p.images?.[0]?.url ?? p.thumbnail ?? '';
                        ...
                        >
                          ...
                          <div className="flex-1 min-w-0">
                            <p className="text-white font-black text-sm leading-tight">
                              {formatPrice(rawPrice)}{isRent ? <span className="text-white/40 font-normal text-xs">/mo</span> : ''}
                            </p>
                            ...
                            {location && (
                              <p className="text-white/35 text-[10px] mt-0.5 flex items-center gap-1">
                                <MapPin className="w-2.5 h-2.5 shrink-0 text-red-400/60" />{location}
                              </p>
                            )}
                          </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="flex items-center justify-between mb-8 flex-wrap gap-3">
            ...
            <div className="flex items-center gap-3">
              <div className="w-1 h-6 rounded-full" style={{ background: 'linear-gradient(to bottom, #e74c3c, #96281b)' }} />
              <p className="text-white/50 text-sm font-medium" style={{ fontFamily: 'monospace' }}>
                {loading ? (
                  <span className="text-white/20">Loading…</span>
                ) : (
                  <>
                    <span className="text-red-400 font-black text-xl" style={{ fontFamily: 'Georgia, serif' }}>
            ...
            <div className="flex items-center gap-3">
              {/* Sort */}
              <div className="flex items-center gap-2 border border-red-900/40 rounded-xl px-3 py-2"
                ...
                >
                  {SORT_OPTIONS.map(o => (
                    <option key={o.value} value={o.value} style={{ background: '#2d0a0a' }}>{o.label}</option>
                  ))}
                </select>
              </div>

              {/* View toggle */}
              <div className="flex border border-red-900/40 rounded-xl overflow-hidden" style={{ background: 'rgba(80,10,10,0.4)' }}>
                <button
          ...
          {loading ? (
            <div className={`grid gap-5 ${viewMode === 'grid' ? 'grid-cols-1 sm:grid-cols-2 lg:grid-cols-3' : 'grid-cols-1'}`}>
              {[...Array(9)].map((_, i) => (
                <div key={i} className="h-72 rounded-2xl animate-pulse border border-red-900/20"
                  style={{ background: 'linear-gradient(135deg, #1e0808 0%, #110404 100%)' }} />
              ))}
            </div>
          ) : properties.length > 0 ? (
            <>
              <div className={`grid gap-5 mb-12 ${viewMode === 'grid' ? 'grid-cols-1 sm:grid-cols-2 lg:grid-cols-3' : 'grid-cols-1'}`}>
              ...
              {pagination && pagination.last_page > 1 && (
                <div className="flex items-center justify-center gap-2 flex-wrap pt-4 border-t border-red-900/20">
                  ...
                  {Array.from({ length: pagination.last_page }, (_, i) => i + 1)
                    ...
                    .map((p, idx) =>
                      p === 'ellipsis'
                        ? <span key={`e-${idx}`} className="px-2 text-white/20" style={{ fontFamily: 'monospace' }}>…</span>
                        : <button
                            key={p}
                            onClick={() => handlePageChange(p as number)}
                            className="w-9 h-9 rounded-xl text-sm font-bold transition border"
                            style={currentPage === p
                              ? { background: 'linear-gradient(135deg,#c0392b,#96281b)', borderColor: 'rgba(192,57,43,0.5)', color: 'white', fontFamily: 'monospace' }
                              : { background: 'rgba(80,10,10,0.35)', borderColor: 'rgba(180,30,30,0.25)', color: 'rgba(255,255,255,0.5)', fontFamily: 'monospace' }
                            }
                          >{p}</button>
                    )}
            ...
            <div className="rounded-2xl p-16 text-center border border-red-900/30"
              style={{ background: 'linear-gradient(135deg, #1e0808 0%, #110404 100%)' }}>
              ...
              <p className="text-2xl text-white font-black mb-2" style={{ fontFamily: 'Georgia, serif' }}>No properties found</p>
              <p className="text-white/30 text-sm mb-6">Try adjusting your filters or clearing the active filter</p>
              {quickFilter && (
                <button
...
export default function PropertiesPage() {
  return (
    <Suspense fallback={
      <div className="w-full min-h-screen flex items-center justify-center" style={{ background: '#150808' }}>
        <div className="flex flex-col items-center gap-4">
          <div className="w-10 h-10 rounded-full border-2 border-red-600 border-t-transparent animate-spin" />
          <p className="text-white/30 text-xs tracking-widest uppercase" style={{ fontFamily: 'monospace' }}>
            Loading properties
          </p>
        </div>
      </div>
