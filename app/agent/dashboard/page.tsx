...
import {
  ...
  Heart, LogOut, ArrowRight, Loader2, Phone, Mail, MapPin, Home,
  Clock, CheckCircle2, Eye, XCircle, Search, RefreshCw, User,
  Bed, Bath, Ruler, Trash2, Edit2, TriangleAlert, Plus, Video,
  ChevronLeft, ChevronRight, SlidersHorizontal, Inbox, Camera,
  Save, AlertCircle, Star,
} from 'lucide-react';

type ActivePanel = 'overview' | 'inquiries' | 'tours' | 'listings' | 'reviews' | 'profile';
    ...
    <div className="space-y-6">
      ...
      <div className="glass rounded-2xl border border-white/10 p-5">
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <Link href="/list-property" className="flex items-center gap-2 px-4 py-3 rounded-xl bg-white/10 hover:bg-white/15 border border-white/10 text-white text-sm font-semibold transition-all">
...
function InquiriesPanel() {
  ...
  const changeStatus = async (id: number, status: InqStatus) => {
    setUpdatingId(id);
    try {
      await fetch(`/api/inquiries/${id}`, { method: 'PATCH', credentials: 'include', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ status }) });
  ...
  const displayed = (data?.data ?? []).filter(i => {
    ...
    return i.lead_name.toLowerCase().includes(q) || i.lead_phone.includes(q) || (i.lead_email ?? '').toLowerCase().includes(q) || (i.property?.title ?? '').toLowerCase().includes(q);
  });
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-5 flex-wrap gap-3">
        <div>
          <div className="flex items-center gap-2">
            ...
            {totalNew > 0 && <span className="text-[11px] font-black px-2.5 py-0.5 rounded-full bg-blue-500/20 border border-blue-500/40 text-blue-300 animate-pulse">{totalNew} new</span>}
          </div>
          <p className="text-white/40 text-xs mt-0.5">{data?.total ?? 0} total from buyers</p>
        </div>
        <button onClick={() => fetch_()} className="inline-flex items-center gap-1.5 px-3 py-2 rounded-xl border border-white/10 bg-white/5 text-white/50 hover:text-white text-xs font-semibold transition-all">
            ...
            return (
              <button key={s} onClick={() => { setStatusFilter(s); setPage(1); }}
                className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-[11px] font-bold border transition-all ${
                  statusFilter === s ? (cfg ? `${cfg.pill}` : 'bg-white/10 border-white/20 text-white') : 'border-white/10 text-white/40 hover:text-white/70'
                }`}>
                {cfg && <span className={`w-1.5 h-1.5 rounded-full ${cfg.dot}`} />}
                {cfg ? cfg.label : 'All'} ({s === '' ? (data?.total ?? 0) : (data?.data.filter(i => i.status === s).length ?? 0)})
              </button>
            );
          })}
        </div>
      ...
      {loading ? (
        <div className="flex flex-col items-center py-16 gap-3"><Loader2 className="w-6 h-6 text-blue-400 animate-spin" /><p className="text-white/30 text-sm">Loading…</p></div>
      ) : displayed.length === 0 ? (
        <div className="flex flex-col items-center py-16 gap-3"><Inbox className="w-8 h-8 text-white/20" /><p className="text-white/40 text-sm">No inquiries found</p></div>
      ) : (
            ...
            return (
              <div key={inq.id} className={`glass rounded-xl border overflow-hidden transition-all ${isNew ? 'border-blue-500/30' : 'border-white/10'}`}>
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center justify-between gap-2 mb-1">
                        <div className="flex items-center gap-1.5 flex-wrap">
                          <span className={`inline-flex items-center gap-1 text-[10px] font-bold px-2 py-0.5 rounded-full border ${cfg.pill}`}>{cfg.icon}{cfg.label}</span>
                          {isNew && <span className="text-[9px] font-black px-1.5 py-0.5 rounded-full bg-blue-500/20 border border-blue-500/40 text-blue-300 uppercase animate-pulse">Unread</span>}
                        </div>
                      ...
                      {inq.property && <p className="text-white/35 text-[11px] flex items-center gap-1 mb-2"><MapPin className="w-2.5 h-2.5 flex-shrink-0" />{inq.property.city}</p>}
                      <div className="bg-white/5 border border-white/8 rounded-lg px-3 py-2 flex flex-wrap items-center gap-x-3 gap-y-1.5">
                        <div className="flex items-center gap-1.5"><User className="w-3 h-3 text-white/40" /><span className="text-xs font-bold text-white">{inq.lead_name}</span></div>
                        <a href={`tel:${inq.lead_phone}`} className="flex items-center gap-1 text-[11px] text-white/50 hover:text-emerald-400 transition-colors"><Phone className="w-2.5 h-2.5" />{inq.lead_phone}</a>
                        {inq.lead_email && <a href={`mailto:${inq.lead_email}`} className="flex items-center gap-1 text-[11px] text-white/50 hover:text-blue-400 transition-colors truncate max-w-[150px]"><Mail className="w-2.5 h-2.5 flex-shrink-0" />{inq.lead_email}</a>}
                        <span className="text-[10px] text-white/25 ml-auto capitalize">Prefers {inq.preferred_contact}</span>
                      </div>
                      {inq.viewing_date && (
                        <div className="mt-2">
                          <span className="inline-flex items-center gap-1 text-[11px] font-semibold text-violet-400 bg-violet-500/10 border border-violet-500/30 px-2.5 py-1 rounded-full">
                            <Calendar className="w-3 h-3" />Viewing: {new Date(inq.viewing_date).toLocaleDateString('en-PH', { month: 'short', day: 'numeric', year: 'numeric' })}
                          </span>
                        </div>
                      )}
                      {inq.message && (
                        <div className="mt-2">
                          ...
                          <button onClick={() => setExpanded(isExp ? null : inq.id)} className="text-[10px] text-white/30 hover:text-white/60 flex items-center gap-1 mt-0.5">
                ...
                <div className="px-4 py-2.5 bg-white/3 border-t border-white/8 flex items-center gap-2 flex-wrap">
                  <a href={`tel:${inq.lead_phone}`} className="inline-flex items-center gap-1 text-[11px] font-semibold px-2.5 py-1.5 rounded-lg border border-white/10 bg-white/5 text-white/50 hover:text-emerald-400 hover:border-emerald-500/30 transition-all"><Phone className="w-3 h-3" /> Call</a>
                  {inq.lead_email && <a href={`mailto:${inq.lead_email}`} className="inline-flex items-center gap-1 text-[11px] font-semibold px-2.5 py-1.5 rounded-lg border border-white/10 bg-white/5 text-white/50 hover:text-blue-400 hover:border-blue-500/30 transition-all"><Mail className="w-3 h-3" /> Email</a>}
                    ...
                    {nexts.map(ns => {
                      ...
                      return (
                        <button key={ns} disabled={updatingId === inq.id} onClick={() => changeStatus(inq.id, ns)}
                          ...
                          {updatingId === inq.id ? <Loader2 className="w-2.5 h-2.5 animate-spin" /> : <span className={`w-1.5 h-1.5 rounded-full ${nc.dot}`} />}
                          {nc.label}
                        </button>
                      );
                    })}
      ...
      {data && data.last_page > 1 && (
        <div className="flex items-center justify-between mt-6 pt-4 border-t border-white/10">
          ...
          <div className="flex gap-2">
            <button onClick={() => setPage(p => Math.max(1, p - 1))} disabled={page === 1} className="px-3 py-1.5 rounded-lg border border-white/10 bg-white/5 text-white/50 hover:text-white text-xs font-semibold transition-all disabled:opacity-30"><ChevronLeft className="w-4 h-4" /></button>
            <button onClick={() => setPage(p => Math.min(data.last_page, p + 1))} disabled={page === data.last_page} className="px-3 py-1.5 rounded-lg border border-white/10 bg-white/5 text-white/50 hover:text-white text-xs font-semibold transition-all disabled:opacity-30"><ChevronRight className="w-4 h-4" /></button>
          </div>
        </div>
      )}
    </div>
...
function ToursPanel() {
  ...
  interface Tour {
    ...
    tour_date: string; tour_time: string; preferred_contact: string;
  ...
  const changeStatus = async (id: number, status: TourStatus) => {
    setUpdatingId(id);
    try {
      await fetch(`/api/tours/${id}`, { method: 'PATCH', credentials: 'include', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ status }) });
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-5 flex-wrap gap-3">
        <div>
          <div className="flex items-center gap-2">
            ...
            {totalPending > 0 && <span className="text-[11px] font-black px-2.5 py-0.5 rounded-full bg-amber-500/20 border border-amber-500/40 text-amber-300 animate-pulse">{totalPending} pending</span>}
          </div>
      ...
      {loading ? (
        <div className="flex flex-col items-center py-16 gap-3"><Loader2 className="w-6 h-6 text-amber-400 animate-spin" /><p className="text-white/30 text-sm">Loading…</p></div>
      ) : (data?.data ?? []).length === 0 ? (
        <div className="flex flex-col items-center py-16 gap-3"><Inbox className="w-8 h-8 text-white/20" /><p className="text-white/40 text-sm">No tours found</p></div>
      ) : (
        <div className="space-y-3">
          {(data?.data ?? []).map(tour => {
                  ...
                  <div className="flex gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-1 flex-wrap">
                        <span className={`inline-flex items-center gap-1 text-[10px] font-bold px-2 py-0.5 rounded-full border ${cfg.pill}`}>{cfg.label}</span>
                        <span className={`inline-flex items-center gap-1 text-[10px] font-bold px-2 py-0.5 rounded-full border ${tour.tour_type === 'video' ? 'bg-blue-500/10 border-blue-500/30 text-blue-400' : 'bg-emerald-500/10 border-emerald-500/30 text-emerald-400'}`}>
                          {tour.tour_type === 'video' ? <><Video className="w-2.5 h-2.5" /> Video</> : <><Home className="w-2.5 h-2.5" /> In-Person</>}
                        </span>
                      ...
                      {tour.property && <p className="text-white/35 text-[11px] flex items-center gap-1 mb-2"><MapPin className="w-2.5 h-2.5" />{tour.property.city}</p>}
                      <div className="inline-flex items-center gap-2 bg-amber-500/10 border border-amber-500/20 rounded-lg px-3 py-1.5 mb-2">
                        ...
                        <div>
                          <p className="text-xs font-bold text-amber-300">{new Date(tour.tour_date).toLocaleDateString('en-PH', { weekday: 'short', month: 'short', day: 'numeric' })}</p>
                          <p className="text-[11px] text-amber-400/70">{fmt(tour.tour_time)}</p>
                        </div>
                      </div>
                      <div className="bg-white/5 border border-white/8 rounded-lg px-3 py-2 flex flex-wrap items-center gap-x-3 gap-y-1.5">
                        <div className="flex items-center gap-1.5"><User className="w-3 h-3 text-white/40" /><span className="text-xs font-bold text-white">{tour.lead_name}</span></div>
                        <a href={`tel:${tour.lead_phone}`} className="flex items-center gap-1 text-[11px] text-white/50 hover:text-emerald-400 transition-colors"><Phone className="w-2.5 h-2.5" />{tour.lead_phone}</a>
                        {tour.lead_email && <a href={`mailto:${tour.lead_email}`} className="flex items-center gap-1 text-[11px] text-white/50 hover:text-blue-400 transition-colors truncate max-w-[150px]"><Mail className="w-2.5 h-2.5 flex-shrink-0" />{tour.lead_email}</a>}
                      </div>
                    </div>
                  </div>
                </div>
                <div className="px-4 py-2.5 bg-white/3 border-t border-white/8 flex items-center gap-2 flex-wrap">
                  <a href={`tel:${tour.lead_phone}`} className="inline-flex items-center gap-1 text-[11px] font-semibold px-2.5 py-1.5 rounded-lg border border-white/10 bg-white/5 text-white/50 hover:text-emerald-400 hover:border-emerald-500/30 transition-all"><Phone className="w-3 h-3" /> Call</a>
                  <div className="ml-auto flex items-center gap-1.5 flex-wrap">
                    <span className="text-[10px] text-white/25">Move to:</span>
                    {nexts.map(ns => {
                      ...
                      return (
                        <button key={ns} disabled={updatingId === tour.id} onClick={() => changeStatus(tour.id, ns)}
                          className="inline-flex items-center gap-1 text-[10px] font-bold px-2.5 py-1.5 rounded-lg border border-white/10 bg-white/5 text-white/40 hover:text-white/80 transition-all disabled:opacity-40">
                          {updatingId === tour.id ? <Loader2 className="w-2.5 h-2.5 animate-spin" /> : <span className={`w-1.5 h-1.5 rounded-full ${nc.dot}`} />}
                          {nc.label}
                        </button>
                      );
                    })}
      ...
      {data && data.last_page > 1 && (
        <div className="flex justify-between items-center mt-6 pt-4 border-t border-white/10">
          <p className="text-white/30 text-xs">Page {data.current_page} of {data.last_page}</p>
          <div className="flex gap-2">
            <button onClick={() => setPage(p => Math.max(1, p - 1))} disabled={page === 1} className="px-3 py-1.5 rounded-lg border border-white/10 bg-white/5 text-white/50 hover:text-white text-xs transition-all disabled:opacity-30"><ChevronLeft className="w-4 h-4" /></button>
            <button onClick={() => setPage(p => Math.min(data.last_page, p + 1))} disabled={page === data.last_page} className="px-3 py-1.5 rounded-lg border border-white/10 bg-white/5 text-white/50 hover:text-white text-xs transition-all disabled:opacity-30"><ChevronRight className="w-4 h-4" /></button>
          </div>
        </div>
      )}
    </div>
...
function ListingsPanel() {
  ...
  interface Property {
    ...
    address: string; city: string; bedrooms: number | null; bathrooms: number | null;
    area: string | null; thumbnail: string | null; created_at: string;
  }
  interface Paginated { data: Property[]; current_page: number; last_page: number; per_page: number; total: number; }
...
const res = await fetch(`/api/agents/listings?${p}`, { credentials: 'include' });
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      ...
      setData(prev => prev ? { ...prev, data: prev.data.filter(p => p.id !== id), total: prev.total - 1 } : prev);
  ...
  return (
    <div>
          ...
          <div className="fixed inset-0 z-[160] flex items-center justify-center p-4">
            <div className="glass border border-white/15 rounded-2xl w-full max-w-sm p-6 shadow-2xl">
              <div className="flex gap-4 mb-5">
                <div className="w-10 h-10 rounded-xl bg-red-500/20 border border-red-500/30 flex items-center justify-center flex-shrink-0"><TriangleAlert className="w-5 h-5 text-red-400" /></div>
                <div><p className="text-white font-bold mb-1">Delete Listing</p><p className="text-white/50 text-sm">This will permanently remove the property and all images.</p></div>
              </div>
              <div className="flex gap-3">
                <button onClick={() => setConfirmId(null)} className="flex-1 py-2.5 rounded-xl border border-white/15 text-white/60 hover:text-white text-sm font-semibold transition-all">Cancel</button>
                <button onClick={() => handleDelete(confirmId)} disabled={deletingId === confirmId} className="flex-1 py-2.5 rounded-xl bg-red-600 hover:bg-red-500 text-white text-sm font-bold transition-all flex items-center justify-center gap-2 disabled:opacity-60">
      ...
      <div className="flex items-center justify-between mb-5 flex-wrap gap-3">
        ...
        <Link href="/list-property" className="inline-flex items-center gap-1.5 px-4 py-2 rounded-xl bg-red-600 hover:bg-red-500 text-white text-xs font-bold transition-all shadow-lg shadow-red-600/30">
      ...
      {loading ? (
        <div className="flex flex-col items-center py-16 gap-3"><Loader2 className="w-6 h-6 text-red-400 animate-spin" /><p className="text-white/30 text-sm">Loading…</p></div>
      ) : (data?.data ?? []).length === 0 ? (
        <div className="flex flex-col items-center py-16 gap-4">
          <Building2 className="w-8 h-8 text-white/20" />
          <p className="text-white/40 text-sm">No listings yet</p>
          <Link href="/list-property" className="inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-red-600 hover:bg-red-500 text-white text-xs font-bold transition-all">
                ...
                <div className="flex">
                  <div className="relative w-28 flex-shrink-0">
                    {prop.thumbnail
                      ...
                      : <div className="w-full flex items-center justify-center bg-white/5" style={{ minHeight: 110 }}><Home className="w-6 h-6 text-white/20" /></div>}
                    <div className={`absolute top-2 left-2 text-[9px] font-black px-2 py-0.5 rounded-full uppercase ${prop.listing_type === 'rent' ? 'bg-violet-600/90 text-white' : 'bg-red-600/90 text-white'}`}>
                  ...
                  <div className="flex-1 p-3 min-w-0">
                    <div className="flex items-start justify-between gap-2 mb-1">
                      <div className="min-w-0">
                        <div className="flex items-center gap-1.5 mb-1 flex-wrap">
                          ...
                          <span className="text-[10px] text-white/30 bg-white/5 border border-white/10 px-2 py-0.5 rounded-full capitalize">{prop.property_type}</span>
                        </div>
                        <p className="text-white text-sm font-bold truncate">{prop.title}</p>
                        <p className="text-white/35 text-[11px] flex items-center gap-1"><MapPin className="w-2.5 h-2.5 flex-shrink-0" />{prop.city}</p>
                      </div>
                    ...
                    <div className="flex items-center gap-2">
                      <Link href={`/properties/${prop.id}`} className="inline-flex items-center gap-1 text-[10px] font-semibold px-2.5 py-1.5 rounded-lg border border-white/10 bg-white/5 text-white/50 hover:text-white hover:border-white/20 transition-all"><Eye className="w-3 h-3" /> View</Link>
                      <Link href={`/agent/dashboard/listings/${prop.id}/edit`} className="inline-flex items-center gap-1 text-[10px] font-semibold px-2.5 py-1.5 rounded-lg border border-white/10 bg-white/5 text-white/50 hover:text-blue-400 hover:border-blue-500/30 transition-all"><Edit2 className="w-3 h-3" /> Edit</Link>
...
function ReviewsPanel({ agentId }: { agentId: number }) {
  ...
  const fetch_ = useCallback(async () => {
    ...
    setError(null);
    try {
      const res = await fetch(`/api/agents/${agentId}/reviews?page=${page}`);
      const raw = await res.text();
      let json: any;
      try { json = JSON.parse(raw); } catch { setError('Unexpected response from server.'); return; }
      let list: Review[] = [];
      let total = 0;
      let lastPage = 1;
  ...
  const avg = data && data.data.length > 0
    ? data.data.reduce((s, r) => s + r.rating, 0) / data.data.length
    : 0;

  const ratingBuckets = [5,4,3,2,1].map(n => ({
    star: n,
    count: data?.data.filter(r => r.rating === n).length ?? 0,
  }));
    ...
    <div>
      <div className="flex items-center justify-between mb-5 flex-wrap gap-3">
        <div>
          ...
          <p className="text-white/40 text-xs mt-0.5">{data?.total ?? 0} reviews from buyers</p>
        </div>
        <button onClick={() => fetch_()} className="inline-flex items-center gap-1.5 px-3 py-2 rounded-xl border border-white/10 bg-white/5 text-white/50 hover:text-white text-xs font-semibold transition-all">
          <RefreshCw className="w-3.5 h-3.5" /> Refresh
        </button>
      ...
      {!loading && data && data.total > 0 && (
        <div className="glass rounded-2xl border border-white/10 p-5 mb-5 flex gap-6 flex-wrap">
          <div className="flex flex-col items-center justify-center min-w-[80px]">
            ...
            <p className="text-white/30 text-xs mt-1">{data.total} {data.total === 1 ? 'review' : 'reviews'}</p>
          </div>
          <div className="flex-1 min-w-[160px] space-y-1.5">
            {ratingBuckets.map(b => (
              <div key={b.star} className="flex items-center gap-2">
                <span className="text-[11px] text-white/40 w-3 text-right">{b.star}</span>
                ...
                <div className="flex-1 h-1.5 bg-white/10 rounded-full overflow-hidden">
                  <div className="h-full bg-yellow-400 rounded-full transition-all duration-500"
                    style={{ width: data.total > 0 ? `${(b.count / data.total) * 100}%` : '0%' }} />
                </div>
                <span className="text-[11px] text-white/30 w-4">{b.count}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      {loading ? (
        <div className="flex flex-col items-center py-16 gap-3"><Loader2 className="w-6 h-6 text-yellow-400 animate-spin" /><p className="text-white/30 text-sm">Loading reviews…</p></div>
      ) : error ? (
        <div className="flex flex-col items-center py-16 gap-3">
          <AlertCircle className="w-8 h-8 text-red-400/50" />
          <p className="text-red-400/70 text-sm">{error}</p>
          <button onClick={() => fetch_()} className="text-xs text-white/40 hover:text-white underline">Try again</button>
        </div>
      ) : (data?.data ?? []).length === 0 ? (
        <div className="flex flex-col items-center py-16 gap-3">
          <Star className="w-8 h-8 text-white/15" />
          <p className="text-white/40 text-sm">No reviews yet</p>
          <p className="text-white/25 text-xs">Complete tours to receive buyer reviews</p>
        </div>
      ) : (
        <div className="space-y-3">
          {(data?.data ?? []).map(review => (
            <div key={review.id} className="glass rounded-xl border border-white/10 p-4">
              <div className="flex items-start gap-3">
                {review.buyer?.avatar
                  ? <img src={imgUrl(review.buyer.avatar)} alt={review.buyer?.name ?? ''} className="w-9 h-9 rounded-full object-cover border border-white/20 flex-shrink-0" />
                  : <div className="w-9 h-9 rounded-full bg-white/10 border border-white/20 flex items-center justify-center flex-shrink-0"><User className="w-4 h-4 text-white/40" /></div>
                }
                <div className="flex-1 min-w-0">
                  ...
                  {review.comment && (
                    <p className="text-white/50 text-xs leading-relaxed italic bg-white/5 border border-white/8 rounded-lg px-3 py-2">"{review.comment}"</p>
                  )}
                </div>
              </div>
            </div>
      ...
      {data && (data.last_page ?? 1) > 1 && (
        <div className="flex justify-between items-center mt-6 pt-4 border-t border-white/10">
          ...
          <div className="flex gap-2">
            <button onClick={() => setPage(p => Math.max(1, p - 1))} disabled={page === 1} className="px-3 py-1.5 rounded-lg border border-white/10 bg-white/5 text-white/50 hover:text-white text-xs transition-all disabled:opacity-30"><ChevronLeft className="w-4 h-4" /></button>
            <button onClick={() => setPage(p => Math.min(data.last_page ?? 1, p + 1))} disabled={page === (data.last_page ?? 1)} className="px-3 py-1.5 rounded-lg border border-white/10 bg-white/5 text-white/50 hover:text-white text-xs transition-all disabled:opacity-30"><ChevronRight className="w-4 h-4" /></button>
          </div>
        </div>
      )}
    </div>
...
function ProfilePanel({ user, fetchUser }: { user: any; fetchUser: () => Promise<void> }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (form.phone && !/^09\d{9}$/.test(form.phone)) { setErrorMsg('Phone must be 11 digits starting with 09'); setStatus('error'); return; }
        ...
        <div className="glass rounded-2xl border border-white/10 p-6 text-center">
          <div className="relative inline-block mb-3">
            {displayAvatar
              ...
              : <div className="w-20 h-20 rounded-full mx-auto bg-primary/20 flex items-center justify-center text-2xl font-bold text-primary">{user.name.charAt(0).toUpperCase()}</div>}
          ...
          <div className="mt-4 space-y-2 text-left">
            {user.phone && (
              <div className="flex items-center gap-2.5 bg-white/5 border border-white/8 rounded-xl px-3 py-2.5">
                <Phone className="w-3.5 h-3.5 text-white/30 flex-shrink-0" /><span className="text-xs text-white/60">{user.phone}</span>
              </div>
            )}
            <div className="flex items-center gap-2.5 bg-white/5 border border-white/8 rounded-xl px-3 py-2.5">
              <Mail className="w-3.5 h-3.5 text-white/30 flex-shrink-0" /><span className="text-xs text-white/60 truncate">{user.email}</span>
            </div>
            {user.specialization && (
              <div className="flex items-center gap-2.5 bg-white/5 border border-white/8 rounded-xl px-3 py-2.5">
                <Building2 className="w-3.5 h-3.5 text-white/30 flex-shrink-0" /><span className="text-xs text-white/60">{user.specialization}</span>
              </div>
            )}
            {user.experience_years && (
              <div className="flex items-center gap-2.5 bg-white/5 border border-white/8 rounded-xl px-3 py-2.5">
                <CheckCircle2 className="w-3.5 h-3.5 text-white/30 flex-shrink-0" /><span className="text-xs text-white/60">{user.experience_years} yrs experience</span>
              </div>
            )}
            {user.license_number && (
              <div className="flex items-center gap-2.5 bg-white/5 border border-white/8 rounded-xl px-3 py-2.5">
                <Settings className="w-3.5 h-3.5 text-white/30 flex-shrink-0" /><span className="text-xs text-white/60">PRC: {user.license_number}</span>
              </div>
            )}
          </div>
        </div>
        <form onSubmit={handleSubmit} className="sm:col-span-2 glass rounded-2xl border border-white/10 p-6 space-y-4">
          ...
          <div>
            ...
            <div className="relative">
              <input type="tel" value={form.phone} onChange={e => setForm(p => ({ ...p, phone: e.target.value.replace(/\D/g, '').slice(0, 11) }))}
              ...
              <span className={`absolute right-4 top-1/2 -translate-y-1/2 text-xs font-semibold ${form.phone.length === 11 ? 'text-emerald-400' : 'text-white/30'}`}>{form.phone.length}/11</span>
            </div>
          </div>
...
export default function AgentDashboardPage() {
      ...
      try {
        const [a, b, c, d, e, f] = await Promise.all([
         ...
         fetch('/api/agents/listings?per_page=1', { credentials: 'include' }),
          ...
          fetch(`/api/agents/${user.id}/reviews`),
        ]);
        const [ai, bi, ci, di, ei, fi] = await Promise.all([a,b,c,d,e,f].map(r => r.ok ? r.json() : null));
  ...
  // ✅ replace() only — no refresh() which interferes with navigation
  ...
  const NAV: { id: ActivePanel; icon: React.ReactNode; label: string; badge: number | null }[] = [
    { id: 'overview',  icon: <LayoutDashboard className="w-4 h-4" />, label: 'Overview',    badge: null },
    ...
    { id: 'profile',   icon: <Settings        className="w-4 h-4" />, label: 'Profile',     badge: null },
  ];
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
                ...
                <button key={item.id} onClick={() => setActivePanel(item.id)}
                  ...
                  {item.badge ? (
                    <span className={`text-[10px] font-black px-2 py-0.5 rounded-full border ${
                      item.id === 'listings' ? 'bg-white/10 border-white/20 text-white/70' :
                      item.id === 'reviews'  ? 'bg-yellow-500/20 border-yellow-500/40 text-yellow-300' :
        ...
        <div className="lg:col-span-3">
          ...
          {activePanel === 'inquiries' && <InquiriesPanel />}
